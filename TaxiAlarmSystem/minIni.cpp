...
static int getstring(INI_FILETYPE *fp, const TCHAR *Key, TCHAR *Buffer, int BufferSize)
{
  TCHAR *sp, *ep;
  int len, idx;
  enum quote_option quotes;
  TCHAR LocalBuffer[INI_BUFFERSIZE] = {0};
  ...
  do {
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
  } while (*sp == ';' || *sp == '#' || ep == NULL || (((int)(skiptrailing(ep,sp)-sp) != len || _tcsnicmp(sp,Key,len) != 0)));
...
int	ini_getstr(const mTCHAR *Key, mTCHAR *Buffer, int BufferSize, const mTCHAR *Filename)
{
	INI_FILETYPE fp;
	int ok = 0;
	...
	if (Buffer == NULL || BufferSize <= 0 || Key == NULL)
		return 0;
	if (ini_openread(Filename, &fp)) {
		ok = getstring(&fp, Key, Buffer, BufferSize);
		(void)ini_close(&fp);
	} 
	return _tcslen(Buffer);
}
...
long ini_getlong(const mTCHAR *Key, long DefValue, const mTCHAR *Filename)
{
	TCHAR LocalBuffer[64];
	int len = ini_getstr(Key,LocalBuffer, sizearray(LocalBuffer), Filename);
	return (len == 0) ? DefValue
		: ((len >= 2 && _totupper(LocalBuffer[1]) == 'X') ? _tcstol(LocalBuffer, NULL, 16)
		: _tcstol(LocalBuffer, NULL, 10));
...
//	int s, k;
//	char section[50];
	char *inifile2 = "test.ini";
	//n = ini_gets("first", "string", "dummy", str, sizearray(str), inifile2);
	n = ini_getstr("dummy", str, sizearray(str),inifile2);
	n = ini_getstr("logfile",str, sizearray(str), inifile2);
	n = ini_getstr("loglevel",str, sizearray(str), inifile2);
	n = ini_getstr("port",str, sizearray(str), inifile2);
	n = ini_getstr("collector_host",str, sizearray(str), inifile2);
}
